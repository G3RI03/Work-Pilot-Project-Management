...
export const updateProject = async (req, res) => {
    try{
        const {userId} = await req.auth();
        const{id,workspaceId,name,description,progress,status,start_date,end_date,priority} = req.body;

        //check if user has admin role at workspace
...
export const addMember = async (req,res)=>{
    try{ 
        ...
        if(project.team_lead !== userId){
            return res.status(403).json({message: "Only project lead can add members to project"});
        }
