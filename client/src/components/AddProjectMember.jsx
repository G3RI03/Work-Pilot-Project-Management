...
import api from "../configs/api.js"; // ✅ missing import
import { fetchWorkspaces } from "../features/workspaceSlice"; // ✅ missing import
...
const AddProjectMember = ({ isDialogOpen, setIsDialogOpen }) => {
    ...
    const projectMembersEmails = project?.members?.map((member) => member.user.email) || []; // ✅ fallback to []
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setEmail(''); // ✅ reset selection
            setIsDialogOpen(false);
            dispatch(fetchWorkspaces({ getToken }));
        } catch (error) {
            toast.error(error.response?.data?.message || error.message); // ✅ fixed syntax: "response.?" → "response?."
        } finally {
            setIsAdding(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black/20 dark:bg-black/50 backdrop-blur flex items-center justify-center z-50">
            <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md text-zinc-900 dark:text-zinc-200">
                <div className="mb-4">
                    ...
                    {project && ( // ✅ guard against project being undefined
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit"
                            disabled={isAdding || !project || !email} // ✅ also disable if no project or email selected
