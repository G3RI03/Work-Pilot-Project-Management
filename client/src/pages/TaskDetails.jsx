...
const TaskDetails = () => {
    ...
    const { user } = useUser(); // ✅ was missing destructuring
    ...
    const fetchComments = async () => {
        ...
        try {
            ...
            const { data } = await api.get(`/api/comments/${taskId}`, {
                headers: { Authorization: `Bearer ${token}` } // ✅ fixed: was missing space "Bearer${token}"
            });
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            setNewComment("");
            toast.dismiss(toastId); // ✅ fixed: toast.dismissAll() doesn't exist
                ...
                {project && (
                    <div className="p-4 rounded-md bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-200 border border-gray-300 dark:border-zinc-800">
                        ...
                        <p className="text-xs mt-3">
                            Project Start Date: {project.start_date ? format(new Date(project.start_date), "dd MMM yyyy") : "N/A"} {/* ✅ guard against null date */}
                        </p>
